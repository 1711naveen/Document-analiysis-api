...
def convert_text(text):
    ...
    text = re.sub(r'\b([1-5])\b', lambda match: number_to_word(int(match.group(0))), text)
    # Convert words to numbers
    text = re.sub(r'\b(one|two|three|four|five)\s*(kg|m|cm|g|l)\b', lambda match: str(word_to_number(match.group(1))) + ' ' + match.group(2), text)
    return text
